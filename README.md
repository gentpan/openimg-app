...
从 [Releases](https://github.com/gentpan/openimg-app/releases/latest) 下载
`OpenImg-<版本>.dmg`，打开后把 **OpenImg** 拖进「应用程序」，再从「应用程序」里双击打开。

**别在「下载」里就地双击运行。** 那样 macOS 会把它挂到一个只读的随机路径上跑
（App Translocation），app 连自己在哪都不知道，应用内更新会一直装不上。经「访达」
拖进「应用程序」就不会发生这件事。

（Release 里那个 zip 是给应用内更新用的，手动装请用 dmg。）

第一次打开系统会问一句「确定要打开吗」，点「打开」即可，只问这一次。这是所有
从网上下载的 app 都有的一步。

这个 app 经过 Apple 公证，所以那个弹窗里会写着「Apple 未检测到任何恶意软件」，
并且有一颗可以直接点的「打开」按钮 —— **不需要右键打开，也不用去系统设置里
点「仍要打开」**。

需要 macOS 14 或更新版本，Apple 芯片的 Mac。

装好之后不用再回这个页面：有新版会在应用内提示，点一下「下载并安装」就地更新完，
重开生效。下载物的摘要、签名、版本三样都对得上才会被装上，对不上就不装。
...
**首次打开弹「确定要打开吗」，正常吗？**
正常，点「打开」就行，只问这一次。只要弹窗里写着「Apple 未检测到任何恶意软件」，
就说明这份是完整且经过公证的。

**打开提示「无法验证开发者」怎么办？**
这句话和上面那个弹窗不是一回事，它意味着签名校验没通过 —— 多半是下载没下完，
或者第三方解压工具改动了包内文件。重新下一次 dmg，从盘里拖进「应用程序」。

**应用内更新说装不了？**
把 app 拖进「应用程序」再打开。在「下载」里就地运行的那一份被系统挂在只读路径上，
它没法替换自己 —— 更新前会先检查这一点并说明原因，不会让你白下载一遍。
