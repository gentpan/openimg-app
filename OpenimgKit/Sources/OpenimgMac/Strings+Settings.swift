...
extension SettingsStrings {
    static let zh = SettingsStrings(
        ...
        wmGenNote: "比例锁 1:1、画质锁 1k——水印最终只按画面宽度的十几个百分点渲染，出 4k 只是白花最贵的一档。生成的图带底色，落地后会在本机自动抠掉背景（用系统的前景分割，不额外花额度）；抠不干净可以再点一次「去背景」。与其它 AI 产出一样计入额度、一样进图库，同时把本机这份设成当前水印。",
    ...
    static let en = SettingsStrings(
        ...
        wmGenNote: "Locked to 1:1 and the 1k tier — the watermark is only ever rendered at a low double-digit percentage of the picture's width, so 4K would just spend the priciest tier for pixels nobody sees. The generated image comes with a background; it is cut out on this Mac afterwards using the system's foreground segmentation, at no extra cost. If anything is left behind, hit “Remove Background” again. It counts against your allowance and lands in your library like any other AI image; the local copy is set as the current watermark at the same time.",
